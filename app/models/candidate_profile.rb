class CandidateProfile < ApplicationRecord
  ...
  accepts_nested_attributes_for :work_experiences, allow_destroy: true,
                                                   reject_if: proc { |attrs|
                                                     attrs['job_title'].blank? && attrs['company_name'].blank?
                                                   }
  ...
  validates :phone, format: { with: /\A\+?[0-9\s\-()]{7,20}\z/, message: 'must be a valid phone number' },
                    on: :final_save
  validates :city, :country, presence: true, on: :final_save
  validate :candidate_languages_present, on: :final_save
  ...
  validates :desired_salary, presence: true, numericality: { greater_than: 0 }, if: :shows_salary_field?,
                             on: :final_save
  validates :desired_percentage, presence: true, numericality: { in: 0..100 }, if: :shows_percentage_field?,
                                 on: :final_save
  ...
  def candidate_languages_present
    return unless candidate_languages.reject(&:marked_for_destruction?).empty?

    errors.add(:candidate_languages, 'Please add at least one language')
  end
end
